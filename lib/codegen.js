...
var getViewForSwagger2 = function(opts, type){
    ...
    var data = {
        ...
        eventNamespace: swagger['x-realtime-namespace'],
        methods: [],
        definitions: [],
        events: [],
    };
    
    var responseDefinitions = swagger.responses;
    ...
    _.forEach(swagger.paths, function(api, path){
        ...
        _.forEach(api, function(op, m){
            ...
            _.forEach(params, function(parameter) {
                ...
                parameter.tsType = ts.convertType(parameter, swagger, [parameter.camelCaseName]);
                parameter.cardinality = parameter.required ? '' : '?';
                method.parameters.push(parameter);
            });
            
            _.forEach(op.responses, function(response, name) {
                if(response['x-exclude-from-bindings'] === true) {
                    return;
                }
                if (_.isString(response.$ref)) {
                    var segments = response.$ref.split('/');
                    response = swagger.responses[segments.length === 1 ? segments[0] : segments[2] ];
                }
                response.name = name;
                response.tsType = ts.convertType(response, swagger);
                response.isDefault = name == 'default';
                response.is200 = name == '200';
    ...
    _.forEach(swagger['x-realtime-events'], function(event, name) {
        //Ignore events which contain the x-exclude-from-bindings extension
        if(event['x-exclude-from-bindings'] === true) {
            return;
        }
        
        event.name = name;
        event.camelCaseName = _.camelCase(event.name);
        event.description = event.description || '';
        
        var params = [];
        if(_.isArray(event.parameters)) {
    ...
    _.forEach(swagger.definitions, function(definition, name){
        var eventBindings = {};
        _.forEach(definition['x-realtime-bindings'], function(binding, type) {
            eventBindings[type] = {name:binding, ref: data.events.filter(e => e.name === binding).pop() };
        });
